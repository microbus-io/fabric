...
The pinning mechanism is a regression-test golden table in `subjects_test.go` covering representative cases (multi-segment and hyphenated hostnames, root paths, path arguments, greedy paths, wildcard ports, lowercasing). Any change to the layout has to update those goldens, and the diff in the test file is what code review catches.
...
Regression tests in `metrics_test.go` and `logger_test.go` cover the fast connection-refused path, the slow-timeout path that exercises the spec-defined timeout, and the logs signal.

### Shared OTLP connection

OTLP exporters in one executable targeting the same endpoint/protocol/TLS-material share one reference-counted connection (`otel.go`, `acquireOTLPConn`/`releaseOTLPConn`), collapsing up to 3N connections (N microservices × three signals) to one per distinct target. Per-signal teardown flushes its provider *then* releases, so the final batch still exports. Two invariants keep sharing safe:

- **The connector owns the connection, not the SDK.** Exporters get it via `WithGRPCConn`/`WithHTTPClient`, and the SDK never closes a caller-provided connection, so one connector's shutdown cannot pull it from under another still holding a reference.
- **Dialing is lazy** (connect on first export, not construction), preserving the non-blocking `Startup`.

Because the connector builds the connection itself (the SDK's `otlpconfig` is `internal/`, with no public factory), `otel.go` reads the OTLP certificate env vars directly to honor a custom CA and mTLS client cert.

### Log enrichment and fan-out live in the handler

`LogDebug/LogInfo/LogWarn/LogError` are thin delegations to the `*slog.Logger` from `Logger()`; all enrichment lives in `logHandler`, the root slog handler, so a record logged through `Logger()` directly is enriched identically. The `*Context` methods carry the span and actor in the context; the non-context methods carry a background context and pass through un-enriched (intended).

`logHandler.Handle` does the context work once - mirror the record onto the active span as an event (non-PROD), increment `microbus_log_messages`, dump errors to stderr in developer deployments - then fans out to the deployment terminal handler (colorful in LOCAL, text in TESTING, JSON in LAB/PROD) and, when configured, an `otelslog` bridge. The `MICROBUS_LOG_DEBUG` gate lives in `Enabled`, so a gated debug record is never counted or exported. The trace id is a `trace` attribute only on the console leg (the OTLP leg gets native `TraceId`/`SpanId` from the bridge); span-event mirroring is kept alongside OTLP logs because it survives the PROD tail sampler.

On shutdown the final "Shutdown" log is recorded while the meter and tracer are still live (it feeds both a counter and a span event), then providers tear down in reverse of init order: logs, traces, metrics. Tearing the meter or tracer down first would silently drop that last log's counter increment and span event.
...
### `MeterProvider()` injects identity attributes

An OTEL **resource** attribute (`service.name`, …) is *not* a label on each metric series - the OTLP/Prometheus pipeline splits it into a separate `target_info` series reachable only by a join. So identity that must be filterable per series (`service`, `deployment`, `plane`, `ver`, `id`) has to be a per-measurement attribute. `MeterProvider()` returns `attributedMeterProvider` (`metricsdecor.go`), a decorator that injects those attributes into every measurement, so third-party libraries handed the meter (sequel's `sequel_*`, dwarf's `dwarf_*`) filter uniformly with the connector's own `microbus_*` under a plain selector, no `target_info` join.

The decorator is the **single** injection site - `c.meter` is built from it too - so do not stamp identity by hand in the recording path or it doubles. It wraps synchronous instruments and the `Observer` passed to `RegisterCallback`; a callback registered via the `WithInt64Callback`/`WithFloat64Callback` construction option is *not* intercepted and would emit without identity (neither library uses that form).
...
operator has pulled - trusted for the life of the process, a revocation gap unrelated to the cooldown: even a fresh,
successful fetch would not drop it.
