...
func TestConnector_Log(t *testing.T) {
	...
	assert.Equal(t, "This is a log info message", logs[0].Message)
	assert.Equal(t, zap.WarnLevel, logs[1].Level)
	assert.Equal(t, "This is a log warn message", logs[1].Message)
	assert.Equal(t, zap.ErrorLevel, logs[2].Level)
	assert.Equal(t, "This is a log error message", logs[2].Message)
}
